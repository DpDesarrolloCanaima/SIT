...
DROP DATABASE IF EXISTS `oac_prub`;
CREATE DATABASE IF NOT EXISTS `oac_prub` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `oac_prub`;
...
-- Volcando datos para la tabla oac_prueba.datos_del_dispotivo: ~2 rows (aproximadamente)
...
CREATE TABLE IF NOT EXISTS `datos_del_entregante` (
  ...
  CONSTRAINT `datos_del_entregante_ibfk_7` FOREIGN KEY (`id__datos_del_entregante`) REFERENCES `datos_del_dispotivo` (`id_datos_del_dispositivo`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- Volcando datos para la tabla oac_prueba.usuarios: ~20 rows (aproximadamente)
